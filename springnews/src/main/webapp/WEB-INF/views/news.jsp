	...
	<div id='view_div'>
		...
		<table>
			...
			<tr>
				...
				<td class='<%=vo.getId()%>'><a
					href="/springnews/news?select=true&id=<%=vo.getId()%>&title=<%=vo.getTitle()%>&writer=<%=vo.getWriter()%>&content=<%=vo.getContent()%>"><%=vo.getTitle()%></a></td>
				<td class='<%=vo.getId()%>'><a
					href="/springnews/news?action=listwriter&listwriter=<%=vo.getWriter()%>">
						<%=vo.getWriter()%></a></td>
		...
		<form Action="/springnews/news" method="GET">
			...
			<select
				...
				<option value="writedate">date</option>
	...
	<div id='write_div' style="display: none">
		...
		<form Action="/springnews/news" method="POST">
			<input type='hidden' name='action' value='insert'> <input
				id='m_writer' type='text' name='writer' placeholder="작성자명을 입력해주세요"><br>
			<input id='m_title' type='text' name='title' placeholder="제목을 입력해주세요"><br>
			<textarea id='m_content' name="content" placeholder="내용을 입력해주세요"></textarea>
			<br> <input id="first" type='submit' value='저장'><input
				id="second" type='reset' value='재작성'><input type="button"
				id="third" value='취소' onclick="display(2)">
		</form>
	</div>
	<%
	...
	<div id='select_div'>
		...
		<form Action="/springnews/news" method="POST">
			<input type='hidden' name='action'
				...
				name='writer' value="<%=request.getParameter("writer")%>"><br>
			<input type='text' name='title'
				value="<%=request.getParameter("title")%>"><br>
			<textarea name="content"><%=request.getParameter("content")%></textarea>
			<br> <input id="first" type='button' onclick="selectNews(null)"
				value='확인'><input id="second" type='submit' value='수정'><input
				id="third" onclick="selectNews(<%=request.getParameter("id")%>)"
